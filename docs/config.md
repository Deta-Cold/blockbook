...
* `ports` – List of ports used by both back-end and Blockbook. Ports defined here are used in configuration templates
    ...
    * `blockbook_public` – Blockbook's public port that is used to comunicate with detahardhardhard wallet (via Socket.IO).
...
to alter built-in text that is specific for detahardhardhard. Text fields that could be updated are:
